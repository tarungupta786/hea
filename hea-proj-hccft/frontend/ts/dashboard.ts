  ...
  try {
    ...
    document.getElementById('alerts-container').innerHTML = `
      <div class="alert alert-critical" style="margin-bottom:24px;">
        ⚠ Cannot reach API at http://localhost:5000 — Connect backend to view insights.
      </div>`;
...
async function admitPatient() {
  ...
  const age       = (document.getElementById('p-age') as HTMLInputElement).value.trim();
  const gender    = (document.getElementById('p-gender') as HTMLSelectElement).value;
  const priority  = (document.getElementById('p-priority') as HTMLSelectElement).value;
  const condition = (document.getElementById('p-condition') as HTMLInputElement).value.trim();
  const ward      = (document.getElementById('p-ward') as HTMLSelectElement).value;
  ...
  try {
    ...
    closeAdmitModal();
    // Clear form
    ['p-name','p-age','p-condition'].forEach(id => (document.getElementById(id) as HTMLInputElement).value = '');
    loadDashboard();
  } catch (e) {
...
async function dischargePatient(id: number, name: string) {
